...
from utils.capacity_calculator import CapacityCalculator, CapacityEstimate
from utils.intelligent_config import IntelligentConfigGenerator
from utils.adaptive_validator import AdaptiveValidator, ValidationResult
...
class ExperimentAgentV2(BaseAgent):
    ...
    def _load_context(self):
        ...
        if device_config_path.exists():
            ...
            self.log(f"Hardware: {self.gpu_count}x {self.gpu_name} ({self.gpu_memory_gb}GB)")
        ...
        if extracted_path.exists():
            ...
            self.max_position_embeddings = hf_config.get("max_position_embeddings", 32768)
            self.model_params_b = extracted.get("memory_estimate", {}).get("parameters_b", 7.0)
    ...
    def run(self, context: Dict[str, Any]) -> AgentResult:
        ...
        return AgentResult(
            ...
            metadata={
                "capacity": self.capacity_estimate.to_dict() if self.capacity_estimate else {},
    ...
    def _calculate_capacity(self) -> bool:
        ...
        self.log(f"Model specs: {self.model_params_b:.1f}B params, {num_layers} layers, {hidden_size} hidden")
    ...
    def _build_and_validate(self) -> bool:
        ...
        self.log(f"  max_num_batched_tokens: {self.base_config['max_num_batched_tokens']:,}")
        ...
        result, final_config = validator.validate(self.base_config, max_retries=self.max_retries)
        ...
        self.log("[REASONING] Validation successful. Agent adapted config from initial estimate.")
        ...
        if final_config.get("max_model_len") != self.base_config.get("max_model_len"):
            ...
            self.log(f"  - max_model_len: {original:,} → {actual:,} (OOM required reduction)")
