...
class VLLMFlagSelector:
    ...
    def _load_flags(self):
        """Load dynamically extracted flags with fallback."""
    ...
    def _load_builtin_fallback(self):
        """Load minimal built-in flag definitions when JSON not available."""
        self.flags = {
            "max_model_len": {"type": "VALUE", "help": "Maximum model context length"},
            "tensor_parallel_size": {"type": "VALUE", "help": "Tensor parallel size"},
            ...
            "max_num_seqs": {"type": "VALUE", "help": "Maximum concurrent sequences"},
            "dtype": {
                "type": "CHOICE",
            ...
            "kv_cache_dtype": {
                "type": "CHOICE",
                "choices": ["auto", "fp8", "fp8_e4m3", "fp8_e5m2", "fp16"],
            },
            "attention_backend": {
                "type": "CHOICE",
                "choices": ["FLASHINFER", "FLASH_ATTN", "XFORMERS"],
            },
        ...
        self.categories = {
            ...
            "precision": ["dtype", "kv_cache_dtype"],
        }

        self.oom_flags = {
            "max_num_batched_tokens": "Reduce to lower memory usage",
            "max_num_seqs": "Reduce concurrent sequences",
            "gpu_memory_utilization": "Lower to leave more headroom",
        ...
        self.profile_recommendations = {
            ...
            "balanced": {
                ...
                "attention_backend": {"value": "FLASHINFER", "reason": "Good balance"},
            },
            "quality": {
                "enable_chunked_prefill": {"value": False, "reason": "Lower latency"},
                "attention_backend": {
                    "value": "FLASH_ATTN",
                    "reason": "Stable quality",
                },
            },
        }
    ...
    def get_applicable_flags(
        ...
        for flag_name, rec in profile_recs.items():
            ...
            if flag_name == "kv_cache_dtype" and value in [
                "fp8",
                "fp8_e4m3",
                "fp8_e5m2",
            ]:
                if not gpu_type or not any(
                    g in gpu_type.upper() for g in self.FP8_CAPABLE_GPUS
                ):
        ...
        recommendations.sort(
            key=lambda r: self._get_flag_priority(r.flag_name), reverse=True
        )
    ...
    def _get_expected_impact(self, flag_name: str, value: Any) -> str:
        ...
        impacts = {
            (
                "kv_cache_dtype",
                "fp8",
            ): "50% KV cache memory reduction, slight quality trade-off",
            (
                "kv_cache_dtype",
                "fp8_e4m3",
            ): "50% KV cache memory reduction, standard FP8 format",
            ("enforce_eager", False): "CUDA graphs for better performance",
            ("enforce_eager", True): "Eager mode for debugging, slightly slower",
            (
                "enable_chunked_prefill",
                True,
            ): "Better throughput, interleaved prefill/decode",
    ...
    def get_oom_mitigation_strategies(
        ...
        if (
            not current_config.get("enable_chunked_prefill", True)
            and oom_iteration <= 2
        ):
            strategies.append(
                OOMMitigationStrategy(
    ...
    def suggest_next_config(
        ...
        if error_type == "cuda_oom":
            ...
            return (
                new_config,
                f"Context too large, reducing from {current_context} to {new_context}",
            )
    ...
    def get_flag_info(self, flag_name: str) -> Optional[Dict[str, Any]]:
        ...
        flag_info["categories"] = [
            cat for cat, flags in self.categories.items() if flag_name in flags
        ]
